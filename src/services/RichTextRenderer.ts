...
import Heading from "$components/posts/Heading.svelte";
import InlineHyperLink from "$components/posts/InlineHyperLink.svelte";
import Paragraph from "$components/posts/Paragraph.svelte";
  ...
  const options = {
    renderNode: {
      ...
      [BLOCKS.EMBEDDED_ENTRY]: (node) => ({
        ...
        props: {
          node: node,
          inlineContent: inlineContent.find(
            (cnt) => cnt?.sys?.id == node?.data?.target?.sys?.id
          ),
        },
      }),
